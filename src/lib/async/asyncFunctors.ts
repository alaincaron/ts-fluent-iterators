...
export abstract class AsyncFunctor<T, R> {
  ...
  andThen<V>(after: EventualFunctorLike<R, V>): AsyncFunctor<T, V> {
    return AsyncFunctionalFunctor.from(Functions.async_compose(this.f, AsyncFunctor.getFunction(after)));
  }

  compose<V>(before: EventualFunctorLike<V, T>): AsyncFunctor<V, R> {
    return AsyncFunctionalFunctor.from(Functions.async_compose(AsyncFunctor.getFunction(before), this.f));
  }
