...
import { alwaysFalse, alwaysTrue, defaultComparator, lengthComparator } from '../../../src/lib/functions';
...
describe('AsyncFluentIterator', () => {
  ...
  describe('count', () => {
    ...
    it('should use default true predicate', async () => {
      expect(await asyncIterator(range(1, 4)).count(alwaysTrue)).equal(3);
    });
    it('should return 0', async () => {
      expect(await asyncIterator(range(1, 4)).count(alwaysFalse)).equal(0);
    });
  });
