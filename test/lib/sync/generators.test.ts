import { range, repeat, iterator } from '../../../src/lib/sync';
...
describe('Generators', () => {
  ...
  describe('repeat', () => {
    it('should yield the exact number of items', () => {
      expect(iterator(repeat(i => i + 1, 0, 5, 1)).collect()).deep.equal([1, 2, 3, 4, 5]);
    });
    it('should yield powers of 2', () => {
      expect(iterator(repeat(x => 2 ** x, 0, 5, 1)).collect()).deep.equal([1, 2, 4, 8, 16]);
    });
  });
});
